...
# Bind the worker queue, Header Exchange use the key-value in header as route key
if queue.bind('headers-rpc-requests',
              # if 'x-match' set to 'any', only 1 key-value in header matches, then route to the queue
              # if 'x-match' set to 'all', all key-value in header match, then route to the queue
